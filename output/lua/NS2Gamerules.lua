...
if Server then
    ...
    function NS2Gamerules:SetGameState(state)
        ...
        if state ~= self.gameState then
            ...
            local frozenState = (state == kGameState.Countdown or state == kGameState.BuyTime) and (not Shared.GetDevMode())
            self.team1:SetFrozenState(frozenState)
            self.team2:SetFrozenState(frozenState)
            
            if self.gameState == kGameState.BuyTime then
                
                PostGameViz("Game buy time")
    ...
    function NS2Gamerules:OnCommanderLogin(commandStructure, newCommander)
        ...
        elseif not Shared.GetCheatsEnabled() and not self.botTraining and newCommander:GetIsRookie() then
    ...
    function NS2Gamerules:FindTheChosenOne()
        ...
        for index, player in pairs(playerList) do
            ...
            for index, pastVIPSteamId in ipairs(self.listOfPastVIPs) do
                ...
                if pastVIPSteamId and pastVIPSteamId == player:GetSteamId() then
                    isPlayerInList = true
                end   
    ...
    function NS2Gamerules:BeginNewRound()
        ...
        if winConditionMet < 1 and not Shared.GetCheatsEnabled() then
            ...
            for index, vip in pairs(self.listOfPastVIPs) do
                listOfVIPsCount = listOfVIPsCount + 1
                Shared.Message(tostring(vip))
            end
            
            if listOfVIPsCount ~= 0 then
                if self.team1:GetNumPlayers() <= listOfVIPsCount then
                    Shared.Message("Clearing past VIP list")
                    self.listOfPastVIPs = {}
                end
            end
    ...
    function NS2Gamerules:ResetGame()
        ...
        self.startingLocationTeam1 = team1TechPoint
        self.startingLocationTeam2 = team2TechPoint
    ...
    function NS2Gamerules:CastVoteByPlayer(voteTechId, player)
        ...
        elseif voteTechId == kTechId.VoteDownCommander1 or voteTechId == kTechId.VoteDownCommander2 or voteTechId == kTechId.VoteDownCommander3 then
            ...
            if playerIndex <= table.count(commanders) then
                ...
                local target = commanders[playerIndex]
                local team = player:GetTeam()
    ...
    function NS2Gamerules:GetReservedSlots()
        ...
        for t = 1, #tags do
            if string.find(tags[t], "R_S") then
                return tonumber(string.sub(tags[t], 4))
            end
        end
    ...
    function NS2Gamerules:OnUpdate(timePassed)
        ...
        if Server then
            ...
            if self:GetMapLoaded() then
                ...
                self:CheckGameStart()
                self:CheckGameEnd()
                if self.team1:GetNumPlayers() > 0 and self.team2:GetNumPlayers() > 0 then
                    self:CheckRoundEnd()
                end
    ...
    function NS2Gamerules:EndRound(winningTeam, roundNumber)
    ...
    function NS2Gamerules:GetCanJoinTeamNumber(player, teamNumber)
        ...
        if not Shared.GetCheatsEnabled() and not self.botTraining and player:GetPlayerLevel() ~= -1 then
            if not self.gameInfo:GetRookieMode() and not playedTutorial[player:GetSteamId()] and player:GetPlayerLevel() < 1 then
                return false, 1
            elseif self.gameInfo:GetRookieMode() and player:GetPlayerLevel() >= kRookieLevel then
                return false, 2
            end
        end
    ...
    function NS2Gamerules:CheckRoundEnd()
        if self:GetGameStarted() and not Shared.GetCheatsEnabled() and not self:GetGameTeam1WonRound() and not self:GetGameTeam2WonRound() then
            ...
            //Update the clock
            local currentTimeLeft = kRoundLength - (Shared.GetTime() - self.roundStartTime)
            
            local currentTimeLeftSeconds = math.ceil(currentTimeLeft)
            
            self.gameInfo:SetCurrentTimeLeft(currentTimeLeftSeconds)
    ...
    function NS2Gamerules:GetWinConditionMet(marineWins, alienWins)
        if (marineWins > 5) and ((marineWins - alienWins) >= 2) or self.team2:GetNumPlayers() < 1 then
            return 1
        elseif (alienWins > 5) and ((alienWins - marineWins) >= 2) or self.team1:GetNumPlayers() < 1 then
            return 2
        else 
            return 0
        end
    ...
    function NS2Gamerules:CheckGameEnd()
        ...
        PROFILE("NS2Gamerules:CheckGameEndCheckGameEnd")
    ...
    function NS2Gamerules:GetBuyTime()
        return self:GetGameState() == kGameState.BuyTime
    end
    ...
    function NS2Gamerules:UpdateBuyTime(timePassed)
        
        if self:GetGameState() == kGameState.BuyTime then
            
            self.team1:SetFrozenState(true)
            self.team2:SetFrozenState(true)
            
            self.buytime = self.buytime - timePassed
...
function NS2Gamerules:GetGameStarted()
    return self.gameState == kGameState.Started or self.gameState == kGameState.BuyTime
end
