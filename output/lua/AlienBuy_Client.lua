...
function AlienBuy_GetTechAvailable(techId)
    ...
    --local techNode = Client.GetLocalPlayer():GetTechTree():GetTechNode(techId)
...
function GetPurchaseableTechIds(techId)
    ...
    if techTree ~= nil then
        ...
        for _, value in ipairs(addOnCopy) do
...
local function FilterInvalidUpgradesForPlayer(player, forAlienTechId)
    ...
    local validIds = { }
    for _, upgradeTechId in ipairs(techIdTable) do

        if table.icontains(validAddons, upgradeTechId) then
...
local function PurchaseTechs(purchaseIds)
    ...
    assert(purchaseIds)
    assert(table.icount(purchaseIds) > 0)
